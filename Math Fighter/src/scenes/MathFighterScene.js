...
export default class MathFighterScene extends Phaser.Scene {
  ...
  init() {
    ...
    this.numberArray = []; //menyimpan number yang di klik
    this.number = 0;
    this.question = [];
    ...
    this.scoreLabel = 0;
    this.timerLabel = undefined; //teks yang muncull
    this.countdownTimer = 10; // t
    this.timedEvent = undefined; //delay
    this.hitung = undefined;
  }
  preload() {
    ...
    this.load.spritesheet("countdown", "images/countdown.png", {
      frameWidth: 1071,
      frameHeight: 1071,
    });
  }
  ...
  create() {
    ...
    this.hitung = this.physics.add
      .sprite(this.gameHalfWidth, this.gameHalfWidth, "countdown")
      .setScale(0.1)
      .setVisible(false)
      .setVelocity(0, 0);
    ...
    this.timerLabel = this.add
      .text(this.gameHalfWidth + 200, 26, null)
      .setDepth(5);
  }
  ...
  update(time) {
    ...
    if (this.correctAnswer === true && !this.playerAttack) {
      ...
      this.time.delayedCall(500, () => {
        this.createSlash(this.player.x + 60, this.player.y, 4, 600); //tanya
      });
  ...
  createAnimation() {
    ...
    this.anims.create({
      key: "countdown",
      frames: this.anims.generateFrameNumbers("countdown", {
        start: 0,
        end: 2,
      }),
      frameRate: 0.8,
  ...
  generateQuestion() {
    ...
    //gak bisa ngasi pertanyaan minus
    if (operator === "-") {
      if (numberB > numberA) {
    ...
    //pertanyaan biar jawabannya gak ada komanya
    if (operator === ":") {
      do {
  ...
  spriteHit(slash, sprite) {
    ...
    if (sprite.texture.key == "player") {
      sprite.anims.play("player-hit", true); // jika player diserang
      // @ts-ignore
      ...
      sprite.anims.play("enemy-hit", true); //jika enemy diserang
      // @ts-ignore
